...
template <typename T>
T test_kgemm_nn_batched(int const mm, int const nn, int const kk,
...
#ifdef USE_GPU
  {
    ...
    kgemm_nn_batched<T><<<batchCount, nthreads>>>(
...
#else
  {
    kgemm_nn_batched<T>(mm, nn, kk, alpha, ddAarray_, dldAarray_, ddBarray_,
