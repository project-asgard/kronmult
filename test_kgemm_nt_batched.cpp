...
template <typename T>
T test_kgemm_nt_batched(int const mm, int const nn, int const kk,
                        int const batchCount, int const idebug = 1) {
  // ----------------------------------------
  ...
  for (int ibatch = 0; ibatch < batchCount; ibatch++) {
    ...
    auto A = [&](int const i, int const j) -> T & {
      return (A_[indx2f(i, j, ldA)]);
    };

    auto B = [&](int const i, int const j) -> T & {
      return (B_[indx2f(i, j, ldB)]);
    };

    auto C = [&](int const i, int const j) -> T & {
...
#ifdef USE_GPU
  {
    ...
    kgemm_nt_batched<T><<<batchCount, nthreads>>>(
        mm, nn, kk, alpha, ddAarray_, dldAarray_, ddBarray_, dldBarray_, beta,
        ddCarray_, dldCarray_, batchCount);
...
#else
  {
    kgemm_nt_batched<T>(mm, nn, kk, alpha, ddAarray_, dldAarray_, ddBarray_,
                        dldBarray_, beta, ddCarray_, dldCarray_, batchCount);
  };
#endif
...
#pragma omp parallel for reduction(max : max_abserr)
  for (int ibatch = 0; ibatch < batchCount; ibatch++) {
    ...
    auto A = [&](int const i, int const j) -> T const & {
      return (A_[indx2f(i, j, ldA)]);
    };

    auto B = [&](int const i, int const j) -> T const & {
      return (B_[indx2f(i, j, ldB)]);
    };

    auto C = [&](int const i, int const j) -> T const & {
      return (C_[indx2f(i, j, ldC)]);
    };
