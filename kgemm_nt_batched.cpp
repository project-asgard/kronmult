...
void kgemm_nt_batched(int const mm, int const nn, int const kk,
  ...
  kgemm_nt_batched<double><<<batchCount, nthreads>>>(
      mm, nn, kk, alpha, Aarray_, ldAarray_, Barray_, ldBarray_, beta, Carray_,
      ldCarray_, batchCount);
#else
  kgemm_nt_batched<double>(mm, nn, kk, alpha, Aarray_, ldAarray_, Barray_,
