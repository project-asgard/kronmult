...
template <typename T>
T test_kronmult_xbatched(int const idim, int const n, int const batchCount,
...
#pragma omp parallel for
...
#ifdef USE_GPU
  {
    ...
    switch (idim) {
    case 1:
      kronmult1_xbatched<T><<<batchCount, nthreads>>>(
          n, dAparray_, lda, dpdZarray_, dpdYarray_, dpdWarray_, batchCount);
      break;
    case 2:
      kronmult2_xbatched<T><<<batchCount, nthreads>>>(
          n, dAparray_, lda, dpdZarray_, dpdYarray_, dpdWarray_, batchCount);
      break;
    case 3:
      kronmult3_xbatched<T><<<batchCount, nthreads>>>(
          n, dAparray_, lda, dpdZarray_, dpdYarray_, dpdWarray_, batchCount);
      break;
    case 4:
      kronmult4_xbatched<T><<<batchCount, nthreads>>>(
          n, dAparray_, lda, dpdZarray_, dpdYarray_, dpdWarray_, batchCount);
      break;
    case 5:
      kronmult5_xbatched<T><<<batchCount, nthreads>>>(
          n, dAparray_, lda, dpdZarray_, dpdYarray_, dpdWarray_, batchCount);
      break;
    case 6:
      kronmult6_xbatched<T><<<batchCount, nthreads>>>(
          n, dAparray_, lda, dpdZarray_, dpdYarray_, dpdWarray_, batchCount);
      break;
    default:
      expect(false);
    ...
    for (int ibatch_table = 0; ibatch_table < size_batch_table;
      ...
      for (int in_table = 0; in_table < size_n_table; in_table++) {
        ...
        double const max_abserr =
            test_kronmult_xbatched<double>(idim, n, batchCount, idebug);
